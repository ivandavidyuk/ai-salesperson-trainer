...
class Settings(BaseSettings):
    ...
    #
    # Выбрана по хвосту задержки, а не по медиане (замер 30.07.2026,
    # 12 заходов на модель из backend-контейнера на DE, payload как в бою):
    #
    ...
    #
    # У прежней 2.5-flash-lite была лучшая медиана из всех и худший хвост
    # на порядок: в живом разговоре 30.07 провайдер дважды замолчал дольше
    # трёх секунд, потолок в llm.py срабатывал в обеих попытках, и ход
    # терялся — менеджеру приходилось повторять реплику. Берём ту же семью
    # (промпт ведёт себя предсказуемо) ценой +240 мс к медиане: ровные
    # 700 мс полезнее 435 мс, которые иногда превращаются в 13 секунд.
    #
