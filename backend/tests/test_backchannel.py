...
BACKCHANNEL_ONLY = [
    ...
    "нгм",        # мычащий вариант
    "угм угу",
    # Слова-подтверждения (подтверждено логами: «угу» → «понял»/«понял понял»)
    "понял",
    "понял, понял",
    "угу, понял",
    "поняяял",    # удлинение → «понял»
    "ладно",
    "хорошо, хорошо",
    "конечно",
]
...
def test_barge_in_threshold():
    ...
    # Слова-подтверждения (в т.ч. раздвоенные) не перебивают
    assert not _would_barge_in("понял, понял")
    assert not _would_barge_in("угу, понял")
    assert not _would_barge_in("поняяял понял")
    ...
    assert _would_barge_in("понял, а сколько стоит")  # ack + вопрос → перебивает
