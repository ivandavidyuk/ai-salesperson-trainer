...
async def _одна(
    ...
    async with семафор:
        ...
        # Сценарий — только для ловушек, где нужна повторяемость. Полный
        # разговор ведёт модель.
        #
        # До 10.08 сценарий брался и для полного разговора, и это делало
        # проверку недействительной: сценарии написаны в июле, а случаи
        # с тех пор пересобраны. Виталию в сценарии продавали лазерную
        # коррекцию за 45 000, тогда как у него пресбиопия и очки за 3 500,
        # и его условие закрытия — «как линзы выдержат пыль на стройке» —
        # сценарий закрыть не мог: там про линзы ни слова. Отказ выглядел
        # свойством пациента, а был свойством текста.
        #
        # Пересборка случаев теперь идёт у каждого клиента, значит сценарии
        # протухали бы постоянно. Модель же читает прайс и анамнез из базы
        имя_сценария = ловушка
