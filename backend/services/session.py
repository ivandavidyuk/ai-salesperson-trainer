...
# Роль пациента для сессии — случай, собранный под организацию этого
# пользователя. Глобального промпта в Patient с 16.09 нет: старт разговора
# без случая отсекает фронт, а COALESCE оставлен на строки, которые могли
# остаться в старых базах. Тем же порядком читает итоговый оценщик — иначе
# он судил бы по офтальмологии разговор, который роль вела про зубы.
_PATIENT_PROMPT_SQL = (
