    ...
    for строка in история:
        ...
        if новое:
            ...
            выданы = (
                sorted(новое)
                if сухой
                # `показано=когда` гасит плашку: это история, а не новость.
                # Без него первый же заход после начисления высыпал бы
                # человеку стопку бейджей за разговоры месячной давности
                else await achievements.выдать(
                    pool, user_id, новое, когда, показано=когда
                )
            )
            уже |= set(выданы)
            for слаг in выданы:
