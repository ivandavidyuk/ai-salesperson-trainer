...
Запуск из папки backend:
    python scripts/sim_conversation.py <промпт.txt> <реплики.txt> [тип] [ходов]
...
Четвёртый аргумент — длина такого прогона в ходах. Нужен там, где критерий
накопительный: на коротком разговоре «пять возражений подряд» не набирается
не из-за менеджера, а из-за обрыва. В бою лимита ходов нет вовсе.
...
# файлом, а живому менеджеру её задавать нечем — берём столько же, сколько
# в наших сценариях, чтобы прогоны были сопоставимы.
#
# Число важнее, чем кажется: у типов с накопительным критерием («пять
# возражений подряд») короткий прогон проваливает этап не по вине менеджера,
# а потому что разговор оборвался. В бою лимита ходов нет вовсе — менеджер
# говорит, пока сам не нажмёт «Завершить», — поэтому длину вынесли в аргумент
_ХОДОВ_У_МОДЕЛИ = 11
...
async def main() -> None:
    ...
    ходы = lines if not менеджер_модель else [None] * ходов
