...
- **RU-сервер** (Timeweb, `5.129.206.63`) — Caddy (HTTPS/WSS), Next.js,
  PostgreSQL. Пользователи ходят на https://podhod.tech.
...
2. **deploy-de** — копирует compose с раннера, затем SSH на DE, `~/ai-trainer`.
...
> **Серверы в GitHub не ходят.** Раньше deploy-ru начинался с `git pull` на RU.
...
>
> На DE лечится другая болезнь тем же приёмом: там compose никогда
> не приезжал сам, его копировали руками, и расхождение молчало.
>
...
> не в `git log`. Руками `git pull` там уже не сделать: `.git` вычищен
> 31.08.2026 — каталог перестал притворяться репозиторием.
...
```
~/ai-trainer/
├── docker-compose.yml   # кладёт деплой из deploy/docker-compose.de.yml
├── .env                 # DOCKERHUB_USER, REDIS_PASSWORD
└── backend.env          # секреты backend (см. backend/.env.production.example)
```

> **Правки `deploy/docker-compose.de.yml` приезжают сами** — с 31.08.2026
> их кладёт раннер, как и на RU. До этого файл копировали руками, и забытая
> копия молчала: изменение уезжало в git и не действовало. `~/ai-trainer`
> по-прежнему не репозиторий — раннер кладёт в него ровно один файл.
>
> На сервере он называется `docker-compose.yml`, а не `.de.yml`: тогда
> команды разбора ниже работают без `-f`. Переименовывает его шаг деплоя —
> scp класть файл сразу под другим именем не умеет.
>
...
```
~/ai-salesperson-trainer/
├── docker-compose.prod.yml   # кладёт деплой
├── deploy/Caddyfile          # кладёт деплой, монтируется в Caddy каталогом
├── .env                      # переменные для подстановки в compose
├── frontend/.env             # env_file контейнера frontend
└── .deployed-sha             # какая версия выкачена
```
...
**Переезд на другое имя делается через оба имени сразу.** `DOMAIN` принимает
список через запятую — `DOMAIN=новое.tech, старое.example`, — и Caddy выписывает
сертификаты на оба. Пока расходится DNS, старый адрес продолжает работать,
а неудача с сертификатом на новый домен не роняет сайт. Второе имя убирается
через несколько дней, когда кэши резолверов обновятся.

`FASTAPI_WS_URL` меняется **в тот же заход**: из него роуты Next выводят
и http-адрес backend для сборки случаев. Забыть его — значит получить сборку,
которая молча уходит в Next вместо FastAPI (так и было 07.08).

Признак, что маршруты живы: `/cases/` и `/ws/` отдают `{"detail":"Not Found"}`
от FastAPI, а не HTML или редирект на `/login` от Next.
...
2. https://podhod.tech открывается, логин работает.
...
# 2. Пациенты — ОБЯЗАТЕЛЬНО: личности, досье, голоса и сложность.
#    Промпты ролей лежат в пресетах отраслей (seed:presets); они и типы
#    тренировки наливаются сами при старте контейнера, но команды рабочие
#    и лишними не будут
docker compose -f docker-compose.prod.yml exec frontend npm run seed:patients
docker compose -f docker-compose.prod.yml exec frontend npm run seed:presets
...
порядок менять нельзя — без пациентов он завершится с ошибкой. `seed:team`
и `seed:demo` заводят организацию «Демо-клиника» с копией пресета
офтальмологии: без организации и её случаев разговор не начнётся, поэтому
`seed:presets` должен пройти раньше (при старте контейнера он идёт сам).

Личности пациентов, случаи пресетов и промпты типов тренировки живут в коде
сидов (`frontend/scripts/`), а не в базе: источник правды — репозиторий.
Сиды **перезаписывают** их при каждом запуске, поэтому править напрямую
в БД бесполезно — изменения потеряются на следующем сиде.
