...
//    (эталон снят с прода). Это защита от того, чтобы правка сборщика тихо
//    изменила поведение персонажа.
// 2. Слой механизма дошёл до каждого пациента дословно. Написали правило один
//    раз — оно применилось ко всем; пациент его не переопределяет.
...
function main(): void {
  ...
  if (same) {
    ...
    // Кладём рядом, чтобы можно было сравнить с прежним:
    //   git show origin/main:frontend/scripts/seed-patients.ts
    writeFileSync("tamara-assembled.txt", tamara, "utf8");
    console.log("  собранный промпт записан в tamara-assembled.txt\n");
  }
