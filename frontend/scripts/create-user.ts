...
async function main() {
  ...
  if (organizations.length === 1) {
    ...
    // Без организации пользователь не сможет начать ни одного разговора:
    // случаи живут в ней. Выдумывать организацию скрипт не должен
    console.error(
      "\nКлиник в базе нет. Сначала npm run seed:presets и npm run seed:demo " +
        "(создаст «Демо-клинику») либо npm run create-demo."
    );
    process.exit(1);
  }
