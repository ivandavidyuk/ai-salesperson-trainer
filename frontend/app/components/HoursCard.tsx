...
interface Hours {
  ...
  resetsAt: string;
  exhausted: boolean;
  /** Демо-организация: карточка не показывается вовсе — остаток и лимит
      выдали бы тихий потолок, а «доступ на сутки» тарифом не является */
  demo: { expiresAt: string | null; expired: boolean } | null;
}
...
export function HoursCard() {
  ...
  // Демо-организации карточка не показывается: «осталось из 3 часов»
  // раскрыло бы тихий потолок демо-доступа
  if (!loaded || !hours || hours.demo) return null;
