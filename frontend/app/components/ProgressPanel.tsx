...
//
// Этапы — пронумерованные строки: так видно, что этапы идут по порядку,
// и подписям хватает ширины. Высоту панель берёт по содержимому и свободную
// высоту колонки не занимает — см. комментарий у карточки.
...
export default function ProgressPanel({
  ...
  return (
    ...
    // содержимого (`min-height: auto` у flex-элемента), и на 1280×800
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      {/* Ужиматься карточка обязана (`min-h-0` + прокрутка), расти выше
          своего содержимого — нет, поэтому `flex-1` здесь не место.
          С ним карточка вытягивалась во всю высоту экрана, и свободную
      ...
      <div className="flex min-h-0 flex-col overflow-y-auto rounded-[14px] border border-line bg-surface-card px-5 py-[18px]">
        ...
        {hasData && (
          <>
            ...
            {/* Ужиматься ниже содержимого нельзя: без `shrink-0` коробка
                на невысоком экране становилась ниже своих строк, строки
                вываливались вниз и ложились поверх блока «По последнему
                разговору» — на 1440×900 на 41px, на 1280×800 на 141px.
                Прокрутка при этом не появлялась, потому что внешний контейнер
                видел укоротившуюся коробку, а не её содержимое. Прокручиваться
                должна панель целиком, поэтому своего `overflow` здесь нет */}
