...
export async function GET(
  ...
  try {
    ...
    // Свой разговор видит менеджер, разговор своего отдела — руководитель:
    // со страницы статистики он открывает разборы подчинённых, ради этого
    // она и нужна. Отдел сверяем обязательно: до 11.09 условие было «или
    // руководитель», без организации, и руководитель одной клиники читал
    // разговор другой, если к нему попадала ссылка.
    //
    // Чужой разговор не отличаем от несуществующего — по ответу нельзя
    // узнать, что сессия с таким id вообще есть.
    const свой = session?.userId === user.id;
    const свойОтдел =
      user.role === UserRole.head &&
      user.organizationId !== null &&
      session?.user.organizationId === user.organizationId;
    const canRead = session && (свой || свойОтдел);
    if (!session || !canRead) {
