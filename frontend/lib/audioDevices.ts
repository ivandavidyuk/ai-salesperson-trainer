...
function шагиРазрешения(): string[] {
  ...
  if (/Android/.test(ua)) {
    return [
      "Нажмите на значок слева от адреса → «Разрешения» → «Микрофон» → «Разрешить»",
      "Если значка нет: меню ⋮ → «Настройки» → «Настройки сайтов» → «Микрофон» — снимите блокировку с этого сайта",
      "Проверьте, что у самого Chrome есть доступ: настройки телефона → «Приложения» → «Chrome» → «Разрешения» → «Микрофон»",
      "Вернитесь и нажмите «Повторить»",
    ];
  }
  return [
  ...
  switch (name) {
    ...
    case "OverconstrainedError":
      return {
        ...
        // На телефоне микрофон есть всегда: «не найден» там значит, что
        // телефон не отдаёт его браузеру, — совет про гарнитуру ни при чём
        text: наAndroid()
          ? "Телефон не отдаёт браузеру микрофон. Проверьте, что у Chrome есть " +
            "доступ к нему (настройки телефона → «Приложения» → «Chrome» → " +
            "«Разрешения» → «Микрофон») и включена плитка «Доступ к микрофону» " +
            "в шторке, — и нажмите «Повторить»."
