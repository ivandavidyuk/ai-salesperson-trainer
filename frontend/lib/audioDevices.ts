...
/**
 * Человекочитаемая причина отказа в микрофоне.
 *
 * Раньше любой сбой захвата выглядел одинаково — общий экран «нет доступа»,
 * из которого нельзя было понять, запрет это, занятое устройство или его
 * отсутствие.
 */
export function micErrorText(error: unknown): string {
  ...
  switch (name) {
    ...
    case "SecurityError":
      return "Браузер запретил доступ к микрофону. Разрешите его в настройках сайта — обычно это значок замка слева от адреса.";
    case "NotFoundError":
    case "OverconstrainedError":
      return "Микрофон не найден. Проверьте, подключён ли он, и выберите устройство из списка.";
    case "NotReadableError":
      return "Микрофон занят другой программой — например, Zoom, Teams или Skype. Закройте её и попробуйте снова.";
    default:
      return "Не удалось включить микрофон. Проверьте устройство и попробуйте снова.";
  }
}
