...
/**
 * Сколько разговоров ЗАГРУЖАЕМ для блока «Прошлые разговоры».
 *
 * Не сколько покажем: блок тянется на всю высоту колонки, и число строк
 * зависит от экрана — на QHD помещается восемь, на ноутбуке пять. Поэтому
 * сервер отдаёт запас, а сколько из него влезает, страница меряет сама
 * (см. `useВлезаетСтрок` в app/page.tsx).
 *
 * Двадцать пять — потолок с запасом на высокий монитор: строка около
 * сорока пяти пикселей, то есть больше тысячи точек списка. Лишние строки
 * не рисуются вовсе, так что цена запаса — только несколько строк в ответе.
 */
const НЕДАВНИХ_РАЗГОВОРОВ = 25;
