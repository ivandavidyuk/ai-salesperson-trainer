...
| | `SCORER_MODEL` (фоновый оценщик) — **обязательно другая модель, чем `LLM_MODEL`**; `FINAL_SCORER_MODEL` (итоговый) — на него запрет не распространяется. Почему — в [DEAL-OUTCOME.md](DEAL-OUTCOME.md) |
...
| `/patients` | библиотека ИИ-пациентов, у руководителя — с досье |
| `/training` | выбор формата тренировки |
| `/achievements` | игровые достижения |
| `/profile` | профиль: фото, личные данные, смена пароля |
| `/stats` | статистика по менеджерам отдела (только руководитель) |
...
| GET | `/api/team/stats` | статистика по отделу (только руководитель) |
...
| `Patient` | персонаж, которого играет ИИ: личность из репозитория, досье для руководителя, голос, сложность |
| `PatientCase` | случай пациента под организацию: промпт роли, анамнез, карточка, возражения, услуга и диагноз |
...
| `SessionReview` | разбор разговора: общая оценка, оценки по этапам сделки, чек-лист по пунктам с номерами реплик, сильная сторона и точка роста |
...
pytest tests/         # чистая логика: поддакивания, чистка речи, метрики проверок
```

### Поведение промптов

Тесты проверяют код, а не то, как разговаривает пациент. За это отвечает
отдельный инструмент — [backend/qa/](backend/qa/README.md): он прогоняет
пациентов по типам тренировки, считает метрики (немые ходы, длина реплики,
повторы) и даёт отдельной модели прочитать расшифровки.

В CI не гоняется: разговоры идут через платного провайдера и занимают время.
Запускается вручную после правок промптов.
